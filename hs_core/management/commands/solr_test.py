...
from hs_core.hydroshare.utils import get_resource_from_shortkey
from pprint import pprint


def harvest_resource(short_id):
    ind = BaseResourceIndex()
    obj = get_resource_from_shortkey(short_id, or_404=False)
...
class Command(BaseCommand):
    help = "Try to harvest a SOLR resource."
    ...
    def handle(self, *args, **options):
        if len(options['resource_ids']) > 0:  # an array of resource short_id to check.
            for rid in options['resource_ids']:
                harvest_resource(rid)
        else:
            for r in BaseResource.objects.all():
                harvest_resource(r.short_id)
